...
//20170812 this is in a different spot
...
public class Bob_Neurite_Tracer_v3 implements PlugIn {
	...
	public void run(String ignored) {
		int showOutput = 1; //added 20141217
		...
		//20170812
		if (false) {
			macroTiffFilename = "/Volumes/fourt/MapManagerData/john/john_tif/s_2689_ch1.tif";
			macroControlFilename = "/Volumes/fourt/MapManagerData/john/john_tif/stackdb/line/s_2689_c0.txt";
			bConvertTo8Bit = "0";
			bDoCurvature = "0";
			bQuitAtEnd = "1";
		}
		...
		if ( controlFileToLoad == null ) {
			OpenDialog od;
			od = new OpenDialog("xxx Select .traces or .swc file...", directory, null );
		...
		if (showOutput==1) {
			System.out.println("Reading control point file:" + macroControlFilename);
			IJ.log("Reading control point file:" + macroControlFilename);
		}
...
//stupid to use int but the tracing engine TracerThread() seems to only work with int???
		...
		for (int i=0; i<rowCount-1; i++) {
			...
			if (doFitRadii==1) {
				...
				// now this
				List<Point3f> pathPointList = result.getPoint3fList();
