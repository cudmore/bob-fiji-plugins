...
//20140405
//i am so sick of reinventing the wheel every time i change this:
//richard spines
//from bSpine.ipf
//new
//	sprintf uCommand1, "%s --run Bob_Neurite_Tracer_v1 \\\"tiffilename=%s controlfilename=%s convertto8bit=%s docurvature=%s quitatend=%s\\\"", fijiPath, tifFileName, controlFile, num2str(convertTo8Bit), num2str(gDoHessian), num2str(gQuitFijiWhenDone)
//old
//	sprintf uCommand1, "%s --run Bob_Test_v2 \\\"tiffilename=%s controlfilename=%s convertto8bit=%s \\\"", fijiPath, tifFileName, controlFile, num2str(convertTo8Bit)
//20140521, remove image scale on load with Calibratio(). Was picking up scale from Zeiss LSM.
...
public class Bob_Neurite_Tracer_v042014 implements PlugIn {
...
//headless
	...
	//imp.show();
		...
		int numRowsInArray = 200; //hard limit, fix this later and make it dynamic
...
//manager.addPath(fitted,true);
		...
		try {
			...
			//File myOutFile2 = new File( outFileName2);
			//manager.exportToCSV(myOutFile2);
			//03/2014
			//IJ.log("saving .csv file(s)");
			//String outFileName3 = macroControlFilename + ".fills";
			//File myOutFile3 = new File( outFileName3);
			//manager.exportAllAsSWC(outFileName3);
