...
gForceBioFormats = 0;
//(1) use bio-formats to get meta data
run("Bio-Formats Importer", "open="+path+" autoscale color_mode=Default view=[Metadata only] stack_order=Default");
...
//(2) just open normally (gets x/y/z correct, loci DOESNOT)
...
//print("printing meta data from getMetadata");
//print(origMetadata);
List.setList(origMetadata);
metaDataAsList = List.getList;
//print(metaDataAsList);
//find 
//startBadIdx=indexOf(metaDataAsList, "ImageDescription")
metaDataAsList = replace(metaDataAsList, "ImageDescription=", "");
//print(metaDataAsList);
origMetadata = metaDataAsList;
			origMetadata += "\n" + "bAlignBatch_v5=51" + "\n";
...
//added 20131210
			...
			//channels
			origMetadata += "bNumChannels=" + actualChannels + "\n"; //20130224, this can be wrong !!!
			//width
...
//check dimensions and fix s neccessary (20131210 was swapping slices for frames???)
